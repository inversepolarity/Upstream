...
import {  isOnRiver, 
          ...
          distanceToT,
          getRiverCurveHeight,
          getRiverPositionWithCurve } from './river.js';
...
export function animate() {
  if (!state.gameOver) {
    ...
    let yCurve = -2.5 * Math.pow(Math.abs(normalizedOffset), 3); // Match river curve depth
...
if (state.isFalling) {
  ...
  if (!state.hyperdrive) {
    ...
    // PHASE 2: Violent launch off the edge (frames 13-20)
      ...
      state.fallPhysics.velocityX = state.fallDirection * state.fallPhysics.launchPower * launchForce;
    ...
    state.trainCubes.forEach((cube, index) => {
      ...
      if (state.fallTimer > delay) {
        ...
        if (adjustedTimer <= STUMBLE_DURATION) {
          ...
          let stumbleWobble = Math.sin(adjustedTimer * 0.6) * cube.userData.fallPhysics.stumbleIntensity;
          ...
          cube.userData.fallPhysics.velocityX = state.fallDirection * cube.userData.fallPhysics.launchPower * launchForce;
